...
def recommended_jobs():
    if not current_user.preferred_location or not current_user.industry or not current_user.skills:
        flash("⚠️ الرجاء تعبئة معلوماتك الشخصية أولاً لعرض الوظائف الموصى بها.", "warning")
        return redirect(url_for('main.jobseeker_profile'))
...
def update_application_status(app_id, new_status):
    ...
    app.status = new_status  # ✅ هنا يتم تغيير الحالة
    db.session.commit()
