'use client'
/**
 * CasesTimelineView — العرض الموحّد للحالات بأسلوب timeline.
 * يحلّ محلّ SupervisorBoard / ManagerBoard / CeoBoard في تبويب الحالات
 * داخل صفحة /followups.
 *
 * - يعرض كل الحالات (RLS يفلتر حسب الدور)
 * - filter chips: نشطة / كل مرحلة / منتهية
 * - بحث بالاسم
 * - زر "رفع تصعيد جديد" (يفتح NewEscalationModal)
 * - Stepper لكل حالة عند التوسعة
 */
import { useEffect, useMemo, useState } from 'react'
import Link from 'next/link'
import { toast } from 'sonner'
import { Loader2, Clock, Filter, BookOpenCheck, Search, Plus } from 'lucide-react'
import { getCases, getCaseWithHistory } from '@/lib/student-cases/db'
import type { CaseWithStudent, CaseWithHistory } from '@/lib/student-cases/types'
import {
  STAGE_SHORT_LABEL, STAGE_COLOR, CASE_STATUS_LABEL, CASE_STATUS_COLOR, timeAgoArabic,
} from '@/lib/student-cases/format'
import CaseStageStepper from './CaseStageStepper'
import NewEscalationModal from '@/components/followups/NewEscalationModal'
...
export default function CasesTimelineView({ profile }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card-static p-3 flex items-center gap-2 flex-wrap">
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button
                ...
                style={
                  active
                    ? { background: 'var(--accent-warm)', color: '#fff', border: '1px solid var(--accent-warm)' }
                    : { background: 'var(--bg-card, #fff)', color: 'var(--text-primary)', border: '1px solid var(--border-soft)' }
                }
              >
                {f.l}
              </button>
        ...
        {isSupervisor && (
          <button
            ...
            className="btn-primary btn-ripple flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white rounded-xl"
          >
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${STAGE_COLOR[c.current_stage]}`}>
                    ...
                    <div className="flex items-center gap-2 text-[11px] flex-wrap" style={{ color: 'var(--text-muted)' }}>
                      ...
                      <span>·</span>
                      <span className="truncate max-w-md" title={c.trigger_reason}>{c.trigger_reason}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Link
                      ...
                      style={{ background: 'rgba(192,138,72,0.14)', color: '#8B5A1E', border: '1px solid rgba(192,138,72,0.30)' }}
                    >
                      تفاصيل ←
                    </Link>
      ...
      {newEscalationOpen && (
        <NewEscalationModal
          profile={profile}
          onCreated={() => { reload(); setNewEscalationOpen(false) }}
...
function StatChip({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="rounded-xl p-3 flex items-center gap-3"
      style={{ background: `linear-gradient(135deg, ${color}12, ${color}04)`, border: `1px solid ${color}26` }}>
      <div className="w-9 h-9 rounded-lg flex items-center justify-center font-bold text-white"
        style={{ background: color, fontSize: 14 }}>
        {value}
      </div>
      <div className="text-[12px] font-semibold" style={{ color: 'var(--text-primary)' }}>{label}</div>
    </div>
  )
}
